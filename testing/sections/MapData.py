...
class Terrain(BaseStruct):
    ...
    layer: int = Retriever(int16, default = -1, min_ver = (1, 36))
...
class MapData(BaseStruct):
    ...
    string_starter1: bytes = Retriever(Bytes[2], default = b"\x60\x0a", min_ver = (1, 36))
    water_definition: str = Retriever(str16, default = "", min_ver = (1, 36))
    string_starter2: bytes = Retriever(Bytes[2], default = b"\x60\x0a", min_ver = (1, 36))
    colour_mood: str = Retriever(str16, default = "Empty", min_ver = (1, 36))
    string_starter3: bytes = Retriever(Bytes[2], default = b"\x60\x0a", min_ver = (1, 40))
    script_name: str = Retriever(str16, default = "", on_write = [update_script_file_path], min_ver = (1, 40))
    lock_coop_alliances_1_41: bool = Retriever(bool8, default = False, min_ver = (1, 41), max_ver = (1, 41))
    collide_and_correct: bool = Retriever(bool8, default = False, min_ver = (1, 36))
    ...
    separator = Retriever(Bytes[4], default = b"\x9d\xff\xff\xff", max_ver = (1, 21))
    unknown3 = Retriever(Bytes[4], default = b"\x0d\xf0\xad\xde")
    unknown4 = Retriever(Bytes[4], default = b"\x02"+b"\x00"*3)
    ai_type: int = Retriever(uint32, default = 0, max_ver = (1, 21))
    no_waves_on_shore: bool = Retriever(bool8, default = False, min_ver = (1, 36))
