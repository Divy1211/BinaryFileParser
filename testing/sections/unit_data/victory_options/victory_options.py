...
from testing.sections.unit_data.victory_options.victory_condition import VictoryCondition
from testing.sections.unit_data.victory_options.victory_point import VictoryPoint


class VictoryOptions(BaseStruct):
    @staticmethod
    def set_conditions_repeat(_, instance: VictoryOptions):
        VictoryOptions.victory_conditions.set_repeat(instance, instance.num_conditions)

    @staticmethod
    def sync_num_conditions(_, instance: VictoryOptions):
        instance.num_conditions = len(instance.victory_conditions)

    @staticmethod
    def set_points_repeat(_, instance: VictoryOptions):
        VictoryOptions.victory_points.set_repeat(instance, instance.num_points)
    ...
    - 3: Disabled
    """
    victory_conditions: list[VictoryCondition] = Retriever(VictoryCondition,                            default_factory = lambda sv: VictoryCondition(sv), repeat = 0)
    total_points: int =                          Retriever(int32,            min_ver = Version((1, 0)), default = 0)
    num_points: int =                            Retriever(int32,            min_ver = Version((1, 0)), default = 0, on_set = [set_conditions_repeat], on_write = [sync_num_conditions])
    ...
    victory_points: list[VictoryPoint] =         Retriever(VictoryPoint,     min_ver = Version((1, 0)), default_factory = lambda sv: VictoryPoint(sv),     repeat = 0)
    # @formatter:on
