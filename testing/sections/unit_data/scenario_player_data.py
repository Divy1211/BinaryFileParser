...
class ScenarioPlayerData(BaseStruct):
    ...
    diplomacy_stances_interaction: list[int] = Retriever(Array16[uint8],           min_ver = Version((1,  0)), default_factory = lambda _: [3, 0, 3, 3, 3, 3, 3, 3, 3])
    """aka relations"""
    diplomacy_stances_ai_system: list[int] =   Retriever(FixedLenArray[uint32, 9], min_ver = Version((1,  8)), default_factory = lambda _: [0, 1, 4, 4, 4, 4, 4, 4, 4])
    """aka unit_diplomacy"""
    colour: int =                              Retriever(uint32,                   min_ver = Version((1, 13)), default = 0)
