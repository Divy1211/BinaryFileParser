...
use crate::types::r#struct::{SerdeDeserializer, Struct};
...
impl<'de, 'a, 'b> DeserializeSeed<'de> for TypeDeserializer<'a, 'b> {
        ...
        D: Deserializer<'de>,
    {
        match &self.0 {
            ...
            BfpType::Struct(struct_) => {
                let base = SerdeDeserializer(struct_, self.1).deserialize(deserializer)?;
...
pub struct OptionSeed<'a, 'b>(TypeDeserializer<'a, 'b>);
pub struct ArraySeed<'a, 'b>(pub TypeDeserializer<'a, 'b>);
pub struct StackedArraySeed<'a, 'b, 'c>(TypeDeserializer<'a, 'b>, &'c StackedArray);
