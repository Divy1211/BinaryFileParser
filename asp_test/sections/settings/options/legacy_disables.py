...
class LegacyDisables(BaseStruct):
    ...
    _num_disabled_units: list[int]              = Retriever(i32,            min_ver = Version(1, 18),                           default = 0,                         repeat = 16, on_write = sync_num_units)
    disabled_unit_ids: list[list[int]]          = Retriever(Array[30][i32], min_ver = Version(1, 18),                           default_factory = lambda _: [-1]*30, repeat = 16)

    _num_disabled_buildings_old: list[int]      = Retriever(i32,            min_ver = Version(1, 18), max_ver = Version(1, 24), default = 0,                         repeat = 16, on_write = sync_num_disabled_buildings_old)
    _num_disabled_buildings: list[int]          = Retriever(i32,            min_ver = Version(1, 25),                           default = 0,                         repeat = 16, on_write = sync_num_disabled_buildings)
