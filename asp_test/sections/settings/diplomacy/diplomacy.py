...
class Diplomacy(BaseStruct):
    # @formatter:off
    player_stances: list[list[int]]                    = Retriever(Array[16][u32],                                               default_factory = lambda _ver: [3]*16,                                      repeat = 16)
    legacy_victory_info: list[list[LegacyVictoryInfo]] = Retriever(Array[12][LegacyVictoryInfo],                                 default_factory = lambda ver: [LegacyVictoryInfo(ver) for _ in range(12)],  repeat = 16)
    """used in aoe1"""
